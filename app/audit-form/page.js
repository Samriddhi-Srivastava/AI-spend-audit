...
export default function AuditForm() {
    ...
    const TOOL_PLANS = {
        ...
        gemini: ["pro", "ultra"],
        cursor: ["pro", "proPlus"],
        windsurf: ["pro", "proPlus"],
        openaiapi: ["payAsYouGo"],
        anthropicapi: ["payAsYouGo"],
    };

    const TOOL_DETAILS = {
        chatgpt: { name: "ChatGPT", color: "from-green-400 to-emerald-600", short: "CG" },
        claude: { name: "Claude", color: "from-orange-400 to-amber-600", short: "CL" },
        copilot: { name: "GitHub Copilot", color: "from-blue-400 to-cyan-600", short: "CP" },
        gemini: { name: "Gemini", color: "from-purple-400 to-pink-600", short: "GM" },
        cursor: { name: "Cursor", color: "from-gray-300 to-gray-500", short: "CS" },
        windsurf: { name: "Windsurf", color: "from-cyan-400 to-blue-500", short: "WS" },
        openaiapi: { name: "OpenAI API", color: "from-slate-400 to-slate-600", short: "OA" },
    ...
    function handleChange(index, e) {
        ...
        if (name === "tool") {
            updatedTools[index] = { ...updatedTools[index], tool: value, plan: "" };
        } else {
            updatedTools[index] = { ...updatedTools[index], [name]: value };
        }
        setTools(updatedTools);
    }

    function addTool() {
        setTools([...tools, { tool: "", plan: "", monthlySpend: "", users: "", useCase: "" }]);
    }
    ...
    async function handleSubmit(e) {
        ...
        const hasEmptyFields = tools.some(
            (t) => !t.tool || !t.plan || !t.monthlySpend
        );
        ...
        try {
            const response = await fetch("/api/summary", {
                ...
                body: JSON.stringify({ auditResult, tools }),
            });
    ...
    function handleReset() {
        setTools([{ tool: "", plan: "", monthlySpend: "", users: "", useCase: "" }]);
        setResult(null);
        setSummary("");
        setAuditId(null);
    ...
    async function handleNotifyMe() {
        if (!leadData.email) {
            alert("Please enter your email in the form below first");
            return;
        }
        await handleLeadSubmit();
        alert("We'll notify you at " + leadData.email + " when new optimizations are available!");
    }
                    ...
                    <div className="max-w-2xl mx-auto bg-gray-900/70 backdrop-blur-xl border border-gray-800 rounded-3xl p-8 shadow-2xl">
                        ...
                        <form onSubmit={handleSubmit} className="flex flex-col gap-8">
                            ...
                            {tools.map((toolData, index) => (
                                <div key={index} className="border border-gray-800 rounded-2xl p-6 bg-black/20">
                                    ...
                                    <div className="grid md:grid-cols-2 gap-5">
                                        ...
                                        <div className="flex flex-col gap-2">
                                            ...
                                            >
                                                ...
                                                <option value="anthropicapi">Anthropic API</option>
                                            </select>
                                        </div>

                                        {toolData.tool && (
                                            <div className="bg-black/30 border border-gray-800 rounded-2xl p-4 flex items-center gap-4 animate-in fade-in duration-300">
                                                <div className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${TOOL_DETAILS[toolData.tool].color} flex items-center justify-center text-black font-bold text-lg shadow-lg`}>
                                        ...
                                        <div className="flex flex-col gap-2">
                                            <label className="text-sm text-gray-300">Monthly Spend ($)</label>
                                            <input
                                                type="number"
                                                name="monthlySpend"
                                        ...
                                        <div className="flex flex-col gap-2">
                                            <label className="text-sm text-gray-300">Number of Users</label>
                                            <input
                                                type="number"
                                                name="users"
                                        ...
                                        <div className="flex flex-col gap-2">
                                            <label className="text-sm text-gray-300">Primary Use Case</label>
                                            <select
                                                name="useCase"
                        ...
                        {!result && !loading && (
                            <div className="mt-8 border border-dashed border-gray-700 rounded-3xl p-10 text-center bg-black/20">
                                ...
                                <div className="mt-4 flex justify-center gap-2 flex-wrap">
                                    <span className="px-3 py-1 bg-gray-800 rounded-full text-xs text-gray-400">Cost Analysis</span>
                                    <span className="px-3 py-1 bg-gray-800 rounded-full text-xs text-gray-400">Optimization Insights</span>
                                    <span className="px-3 py-1 bg-gray-800 rounded-full text-xs text-gray-400">AI Recommendations</span>
                                </div>
                        ...
                        {result && (
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-8 mb-6">
                                ...
                                <div className="bg-gray-900/70 border border-emerald-500/20 rounded-2xl p-5">
                                    ...
                                    <p className="text-emerald-600 text-xs mt-1">${result.annualSavings}/yr</p>
                                </div>
                                <div className="bg-gray-900/70 border border-gray-800 rounded-2xl p-5">
                                    <p className="text-gray-400 text-sm mb-2">Tools Analyzed</p>
                                    <h3 className="text-3xl font-black text-white">{result.toolsAnalyzed}</h3>
                                </div>
                                <div className="bg-gray-900/70 border border-gray-800 rounded-2xl p-5">
                                    <p className="text-gray-400 text-sm mb-2">Optimization Score</p>
                                    <h3 className={`text-3xl font-black ${result.score >= 80 ? "text-emerald-400" : result.score >= 50 ? "text-yellow-400" : "text-red-400"}`}>
                                        {result.score}/100
                                    </h3>
                                </div>
                            </div>
                        ...
                        {result && result.isSpendingWell && result.totalSavings <= 100 && (
                            <div className="mt-4 mb-6 bg-blue-500/10 border border-blue-500/30 rounded-2xl p-6">
                                <div className="flex items-start justify-between gap-4 flex-wrap">
                                    <div>
                                    ...
                                    <button
                                        ...
                                        className="bg-blue-500 hover:bg-blue-400 text-white font-semibold px-6 py-3 rounded-xl transition-all duration-200 text-sm whitespace-nowrap"
                                    >
                                        Notify Me →
                                    </button>
                                </div>
                        ...
                        {result && (
                            <div className="mt-2 space-y-4">
                                ...
                                {result.toolResults.map((toolResult, index) => (
                                    <div key={index} className="bg-gray-900/70 border border-gray-800 rounded-2xl p-5">
                                        <div className="flex items-center justify-between mb-4">
                                            <div>
                                                <h4 className="text-lg font-bold text-white">
                                                    {toolResult.toolName}
                                                    <span className="text-gray-500 text-sm font-normal ml-2 capitalize">
                                                        {toolResult.plan === "payAsYouGo" ? "Pay-as-you-go" : toolResult.plan === "proPlus" ? "Pro+" : toolResult.plan} plan
                                                    </span>
                                                </h4>
                                            ...
                                            <span className={`px-3 py-1 rounded-full text-sm font-semibold ${toolResult.saving > 0 ? "bg-red-500/10 text-red-400 border border-red-500/20" : "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20"}`}>
                                                {toolResult.saving > 0 ? `Save $${toolResult.saving}/mo` : "✓ Optimised"}
                                            </span>
                                        </div>
                                        <div className="mb-3">
                        ...
                        {result && !leadSubmitted && (
                            <div className="mt-6 bg-gray-900/70 backdrop-blur-xl border border-gray-800 rounded-3xl p-6 text-white">
                                <h3 className="text-xl font-semibold mb-1 text-white">Get your full report</h3>
                                ...
                                <form onSubmit={handleLeadSubmit} className="flex flex-col gap-4">
                                    ...
                                    <p className="text-gray-600 text-xs text-center">No spam. Your data is never sold.</p>
                                </form>
                            </div>
                        )}

                        {result && leadSubmitted && auditId && origin && (
                            <div className="mt-6 bg-emerald-500/10 border border-emerald-500/30 rounded-3xl p-6 text-white">
                                <h3 className="text-xl font-semibold mb-1 text-emerald-400">✓ Your audit is saved</h3>
