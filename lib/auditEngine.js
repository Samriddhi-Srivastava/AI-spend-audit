const TOOL_PRICING = {
    ...
    claude: {
        name: "Claude",
        plans: {
            pro: { price: 20, perUser: true, bestFor: ["writing", "research", "coding"] },
    ...
    gemini: {
        name: "Gemini",
        plans: {
            pro: { price: 20, perUser: true, bestFor: ["research", "productivity", "writing"] },
...
const ALTERNATIVES = {
    ...
    coding: {
        ...
        claude: null, // Claude is fine for coding
        copilot: null,
        cursor: null,
    },
...
function auditSingleTool(toolData) {
    ...
    if (alt) {
        ...
        reason = alt.reason;
        issues.push(reason);
        // Switching doesn't always save money but improves fit
        // Only add savings if the alternative is cheaper
    }
    ...
    if (issues.length === 0 && strengths.length > 0) {
        recommendation = "Keep — well optimised";
        reason = `${toolInfo.name} ${plan} is a good match for your use case and team size.`;
    }

    if (!recommendation) {
        recommendation = "Keep — no major issues found";
...
export function generateFallbackSummary(result, tools) {
    ...
    const { totalSpend, totalSavings, annualSavings, score, toolResults } = result;
    ...
    if (score >= 85) {
        return `Your AI stack ($${totalSpend}/mo across ${tools.length} tool(s)) is well optimised. The tools you have selected align with your use cases and team size. No major changes are recommended at this time.`;
    }
